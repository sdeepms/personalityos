...
export class MuAPIAdapter implements GenerationProvider {
  ...
  async generateProductPhotography(
    ...
    const startMs = Date.now()
    console.log('[muapi] generateProductPhotography start')

    const endpoint = `${MUAPI_HOST}/api/v1/ai-product-photography`
    console.log('[muapi] using endpoint:', endpoint)
    ...
    if (!generateRes.ok) {
      const text = await generateRes.text()
      throw new Error(`MuAPI generateProductPhotography failed (${generateRes.status}): ${text}`)
    }
    ...
    while (attempts < MAX_ATTEMPTS) {
      ...
      if (status.status === 'completed') {
        const url = status.output?.url ?? status.outputs?.[0]
        if (!url) throw new Error('MuAPI completed but returned no image URL')
        outputUrl = url
        break
      }

      if (status.status === 'failed') {
        throw new Error(`MuAPI generateProductPhotography failed: ${status.error ?? 'unknown error'}`)
      }
